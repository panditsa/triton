...
// Wave-uniform offset can be routed to the buffer-load `soffset` slot,
// freeing per-lane VGPR adds in inner loops. Off by default during initial
// validation; flip via `AMDGCN_BUFFER_LOAD_SOFFSET=1`.
bool isSoffsetSplitEnabled() {
  static const bool enabled = []() {
    const char *env = std::getenv("AMDGCN_BUFFER_LOAD_SOFFSET");
    return env && env[0] != '0';
  }();
  return enabled;
}
...
BufferEmitter::emitLoadToLds(Type type, Value byteWidth, Value rsrcDesc,
  ...
  // SGPR `soffset` slot (ArgIndex 4) accepts a runtime SGPR. Allow the
...
void BufferEmitter::fillCommonArgs(Type type, Value rsrcDesc,
  ...
  // The supplied index is in elements, not bytes; convert by multiplying by
  // `elementByteWidth` before feeding the buffer intrinsic.
  ...
  Value sgprOffset;
  bool useSplit = false;
  if (isBufferLoad && allowSoffsetSplit && isSoffsetSplitEnabled()) {
